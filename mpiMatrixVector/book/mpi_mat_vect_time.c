...
void Get_dims(int *m_p, int *local_m_p, int *n_p, int *local_n_p, int my_rank,
              int comm_sz, MPI_Comm comm, int argc, char *argv[]);
...
int main(int argc, char *argv[]) {
  ...
  Get_dims(&m, &local_m, &n, &local_n, my_rank, comm_sz, comm, argc, argv);
...
void Get_dims(int *m_p /* out */, int *local_m_p /* out */, int *n_p /* out */,
              ...
              int comm_sz /* in  */, MPI_Comm comm /* in  */, int argc,
              char *argv[]) {
  int local_ok = 1;

  if (my_rank == 0) {
    if (argc < 3) {
      Check_for_error(local_ok, "Get_dims", "enter matrix dimensions", comm);
    }
